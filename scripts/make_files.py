...
with open("generate.makefile", "w") as mfile:
    ...
    for d in data:
        ...
        mfile.write("\t-mkdir -p release/{0}\n\t../scripts/colorscad/colorscad.sh -f -p $(SCAD) -i $< -o $@ -- --enable predictible-output -d output/{0}__{1}.deps  -D FROM_MAKE=1 -D MAKE_MMU=1\n".format(d.basename, d.module))
        # Extra frogs
        mfile.write("release/{0}/{1}.stl: output/{0}__{1}.scad {0}.scad\n".format(d.basename, d.module))
        mfile.write("\t-mkdir -p release/{0}\n\t$(SCAD) -m make --export-format binstl --enable manifold --enable predictible-output -o $@ -d output/{0}__{1}.deps $< -D FROM_MAKE=1 -D MAKE_MMU=0\n".format(d.basename, d.module))
