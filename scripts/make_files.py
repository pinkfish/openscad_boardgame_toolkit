...
with open("generate.makefile", "w") as mfile:
    ...
    for d in data:
        ...
        mfile.write("\t-mkdir -p release/{0}\n\t../scripts/colorscad/colorscad.sh -f -p $(SCAD) -c \"[0.184314, 0.309804, 0.309804, 1]\" -i $< -o $@ --  --backend=manifold --enable predictible-output --enable textmetrics -d output/{0}__{1}.deps  -D FROM_MAKE=1 -D MAKE_MMU=1\n".format(d.basename, d.module))
        # Extra frogs
