...
# LibFile: tests/render_labels.py
#    Helper for rendering labels.py shapes with the *real* PythonSCAD binary. Same shape as
#    tests/render_cap_box.py (labels.py also calls `osuse("BOSL2/std.scad")` at import time, so
...
def render_labels_shape(expr: str, out_png: Path, imgsize: tuple[int, int] = (320, 240)) -> RenderResult:
    """Convenience wrapper: renders a single labels.py expression, e.g.
    `"MakeStripedGrid([20.0, 50.0]).linear_extrude(height=2)"`. `expr` is evaluated with
    labels'/base_bgtk's public names and pythonscad's native primitives already imported into
    scope, and the project root already on sys.path.
