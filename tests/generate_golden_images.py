...
from render_app import find_pythonscad_binary
...
from test_flags_render import (
    FLAGS as FLAGS_SHAPES,
    GOLDEN_DIR as FLAGS_GOLDEN_DIR,
    IMGSIZE as FLAGS_IMGSIZE,
)
...
def main() -> int:
    ...
    # pysolidfive was removed; its render helper and golden set went with it.
    sources = []
    if find_bosl2_scad_dir() is None:
        ...
        # The per-box-type render modules (cap_box, no_lid, sliding_box, slipover_box,
        # slipover_path_box, boxes) were consolidated into test_all_boxes_render.py, which is a
        # geometry smoke test rather than a golden-image one, so they no longer belong here.
        ...
        sources.append((FLAGS_SHAPES, partial(render_flag, imgsize=FLAGS_IMGSIZE), FLAGS_GOLDEN_DIR))
        sources.append(
