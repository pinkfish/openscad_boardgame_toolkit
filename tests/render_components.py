...
# LibFile: tests/render_components.py
#    Helper for rendering components.py shapes with the *real* PythonSCAD binary. Same shape as
#    tests/render_cap_box.py (components.py also calls `osuse("BOSL2/std.scad")` at import time,
...
def render_components_shape(expr: str, out_png: Path, imgsize: tuple[int, int] = (320, 240)) -> RenderResult:
    """Convenience wrapper: renders a single components.py expression, e.g.
    `"RoundedBoxOnLength([30.0, 20.0, 10.0], 7)"`. `expr` is evaluated with components'/
    base_bgtk's public names and pythonscad's native primitives already imported into scope, and
