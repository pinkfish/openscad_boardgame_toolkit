...
# LibFile: tests/render_sliding_box.py
#    Helper for rendering sliding_box.py shapes with the *real* PythonSCAD binary. Same shape as
#    tests/render_cap_box.py (sliding_box.py also calls `osuse("BOSL2/std.scad")` at import
...
def render_sliding_box_shape(expr: str, out_png: Path, imgsize: tuple[int, int] = (320, 240)) -> RenderResult:
    """Convenience wrapper: renders a single sliding_box.py expression, e.g.
    `"SlidingLid([80.0, 60.0])"`. `expr` is evaluated with sliding_box's/base_bgtk's public
    names and pythonscad's native primitives already imported into scope, and the project root
    already on sys.path.
