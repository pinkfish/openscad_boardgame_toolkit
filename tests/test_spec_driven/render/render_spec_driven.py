...
"""Render helper for spec_driven geometry — shells out to the full PythonSCAD binary.

The spec_driven box/pattern/hex-grid geometry uses pybosl2, which only produces real
CSG inside PythonSCAD. These helpers render a spec_driven solid to a PNG and report
...
def render_solid(name: str, body: str, imgsize: tuple[int, int] = (320, 240)) -> RenderResult:
    """Render a spec_driven solid expression to a PNG via PythonSCAD.
