...
class TestEdgeScalers(unittest.TestCase):
    def test_hexagonal_edge_scales_both_axes(self) -> None:
        ...
        out = tesselations.hexagonal_tesselation_generate_edge(pts, side_length=10)
        self.assertEqual(out, [[-5.0, 0.0], [0.0, 2.0], [5.0, 0.0]])

    def test_square_edge_scales_both_axes(self) -> None:
        pts = [[-0.5, 0.1], [0.5, -0.1]]
        out = tesselations.square_tesselation_generate_edge(pts, side_length=4)
...
class TestTesselationSideLine(unittest.TestCase):
    ...
    def test_normal_maps_side_onto_segment(self) -> None:
        out = tesselations.tesselation_side_line(path=[[0, 0], [10, 0]], side=self.SIDE, flip=TESSELATION_LINE_NORMAL)
    ...
    def test_flipped_mirrors_the_bump(self) -> None:
        out = tesselations.tesselation_side_line(path=[[0, 0], [10, 0]], side=self.SIDE, flip=TESSELATION_LINE_FLIPPED)
    ...
    def test_reverse_runs_the_profile_backwards(self) -> None:
        ...
        normal = tesselations.tesselation_side_line(path=[[0, 0], [10, 0]], side=asym, flip=TESSELATION_LINE_NORMAL)
        reverse = tesselations.tesselation_side_line(path=[[0, 0], [10, 0]], side=asym, flip=TESSELATION_LINE_REVERSE)
    ...
    def test_rotated_segment(self) -> None:
        ...
        out = tesselations.tesselation_side_line(path=[[0, 0], [0, 10]], side=self.SIDE, flip=TESSELATION_LINE_NORMAL)
        self.assertAlmostEqual(out[1][0], -2.0, places=9)
        self.assertAlmostEqual(out[1][1], 5.0, places=9)

    def test_rejects_bad_arguments(self) -> None:
        with self.assertRaises(AssertionError):
            tesselations.tesselation_side_line(path=[[0, 0]], side=self.SIDE)
        with self.assertRaises(AssertionError):
            tesselations.tesselation_side_line(path=[[0, 0], [10, 0]], side=[[0, 0]])
...
class TestTesselationPolygon(unittest.TestCase):
    def test_square_with_flat_profiles_reproduces_square(self) -> None:
        ...
        out = tesselations.tesselation_polygon(
    ...
    def test_rejects_mismatched_lengths(self) -> None:
        with self.assertRaises(AssertionError):
            tesselations.tesselation_polygon(
...
class TestLeafOutlinePathBuilder(unittest.TestCase):
    def test_polygon_is_closed_and_symmetric(self) -> None:
        ...
        pts = tesselations.tesselation_leaf_outline_make_polygon(section_height=section_height, section=section)
