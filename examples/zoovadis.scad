...
module SpecialToken(num = 1) {
  ...
  difference() {
    cuboid(
      [
        special_token_length - special_token_diameter / 2,
        special_token_stalk_width,
        special_token_thickness * num,
      ],
      anchor=LEFT + BOTTOM
    );
    translate([special_token_length - special_token_diameter / 2, 0, 0]) rotate([0, 0, 45])
        cuboid(
          [special_token_angle_length, special_token_angle_length, special_token_thickness * num + 1],
          anchor=BOTTOM
        );
  }
}
...
module StandingBox() // `make` me
{
  ...
  color(default_material_colour) difference() {
      cuboid(
        [
          animal_max_width + wall_thickness * 2,
          (animal_thickness + inner_wall) * 6 + wall_thickness * 2,
          animal_max_length - wall_thickness,
        ],
        anchor=BOTTOM + FRONT
      );
      for (i = [0:1:5]) {
...
module LaurelsBox(num_laurels, num_rows = 5, generate_lid = true) {
  ...
  MakeBoxWithSlipoverLid(
    ...
    for (i = [0:1:num_rows - 1]) {
      translate(
        [
          (laurel_diameter + 0.2) / 2 + laurel_diameter * i + 0.5 * i,
          middle_width / 2 + middle_offset / 2,
    ...
    if ($children > 1) {
      ...
      translate(
        [
          ...
          player_box_length - wall_thickness * 4,
          player_box_wall_height - special_token_thickness * 1.3 - 2,
        ]
