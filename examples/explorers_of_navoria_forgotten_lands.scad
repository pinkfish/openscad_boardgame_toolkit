...
module TradingPostWhite(height)
{
    module OneSide()
    {
        stroke(bezier_curve([[trading_post_white_base / 2 - 0.5,
...
{
    MakeBoxWithSlidingLid(width = faction_skill_box_width, length = faction_skill_box_length,
                          height = faction_skill_box_height)
    {
        cuboid([ faction_skill_width + 1, faction_skill_main_length + 0.5, $inner_height + 1 ],
...
{
    MakeBoxWithSlidingLid(width = bits_box_width, length = bits_box_length, height = bits_box_height,
                          lid_on_length = true)
    {
        ...
        translate([ start_order_oval_width, 0, $inner_height - token_thickness - 1 ]) rotate([ 0, 0, 90 ])
            StartOvalFrom(100);
        translate([
            $inner_width - exploration_tiles_width / 2 - 13, $inner_length - exploration_tiles_width / 2 - 5,
        ...
        translate([
            $inner_width - exploration_tiles_width * 3 / 2 - 20, $inner_length - exploration_tiles_width / 2 - 5,
            $inner_height - token_thickness * 2 - 0.9
        ]) CuboidWithIndentsBottom([ exploration_tiles_width, exploration_tiles_length, token_thickness * 2 + 1 ],
                                   finger_holes = [ 0, 4 ], finger_hole_radius = 20);
        translate([
            $inner_width - exploration_tiles_width * 5 / 2 - 27, $inner_length - exploration_tiles_width / 2 - 5,
            $inner_height - token_thickness * 2 - 0.9
        ]) CuboidWithIndentsBottom([ exploration_tiles_width, exploration_tiles_length, token_thickness * 2 + 1 ],
                                   finger_holes = [ 0, 4 ], finger_hole_radius = 20);
...
{
    SlidingBoxLidWithLabel(width = bits_box_width, length = bits_box_length, text_width = 70, text_height = 20,
                           text_str = "Navoria");
}
...
{
    SlidingBoxLidWithLabel(width = tokens_box_width, length = tokens_box_length, text_width = 70, text_height = 20,
                           text_str = "Solo");
