...

{
    translate([ 0, -intern_top_length / 2, 0 ])
        cuboid([ intern_width, intern_length - intern_top_length, height ], anchor = BOTTOM, rounding = 1,
...
{
    SlidingBoxLidWithLabel(width = bottom_boxes_width, length = bottom_boxes_length, text_width = 70, text_height = 20,
                           text_str = "Sky Team", label_rotated = true);
}

module ApproachTracks() // `make` me
{
    MakeBoxWithSlidingLid(width = approach_track_box_width, length = appraoch_track_box_length,
...
{
    SlidingBoxLidWithLabel(width = approach_track_box_width, length = appraoch_track_box_length, text_width = 70,
                           text_height = 20, text_str = "Approach", label_rotated = true);
}
...
{
    CapBoxLidWithLabel(width = dice_box_width, length = dice_box_length, height = dice_box_height, text_width = 60,
                       text_height = 18, text_str = "Dice", label_rotated = true);
}
...
{
    SlidingBoxLidWithLabel(width = buttons_box_width, length = buttons_box_length, text_width = 30, text_height = 15,
                           text_str = "Button");
    translate([ buttons_box_width + 10, 0, 0 ])
    {
        SlidingBoxLidWithLabel(width = buttons_box_width, length = buttons_box_length * 2, text_width = 30,
                               text_height = 15, text_str = "Alerts");

        translate([ buttons_box_width + 10, 0, 0 ])
        {
            SlidingBoxLidWithLabel(width = buttons_box_width, length = buttons_box_length * 2, text_width = 30,
                                   text_height = 15, text_str = "Pengiun");

            translate([ buttons_box_width + 10, 0, 0 ])
            {
                SlidingBoxLidWithLabel(width = buttons_box_width, length = buttons_box_length * 3, text_width = 30,
                                       text_height = 15, text_str = "Plane");

                translate([ buttons_box_width + 10, 0, 0 ])
                {
                    SlidingBoxLidWithLabel(width = buttons_box_width, length = buttons_box_length, text_width = 30,
                                           text_height = 15, text_str = "Marker");
