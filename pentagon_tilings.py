...
def PentagonTesselationArea(
    ...
    data = PentagonTesselationData(
        ...
        first_angle_modifier=first_angle_modifier, second_angle_modifier=second_angle_modifier,
        first_length_modifier=first_length_modifier, second_length_modifier=second_length_modifier,
        third_length_modifier=third_length_modifier, line1=line1, line2=line2, line3=line3,
    )

    bound = square([width, length]).translate([width / 2, length / 2, 0])
    ...
    for yy in range(math.floor(rows) + 1):
        for xx in range(math.floor(cols) + 1):
            ...
            piece = region(data.points).translate([dx - pentagon_size * 2, dy - pentagon_size * 2, 0])
            shape = piece if shape is None else shape | piece
    shape = shape.rotate([0, 0, spin]).translate([width / 2, length / 2, 0])
...
def PentagonTesselation(
    ...
    return region(data.points).translate([dx, dy, 0])
...
def PentagonTesselationData(
    ...
    assert -60 <= first_angle_modifier <= 60, f"Invalid first angle modifier first_angle_modifier={first_angle_modifier}"
    assert -60 <= second_angle_modifier <= 60, f"Invalid second angle modifier second_angle_modifier={second_angle_modifier}"
    assert -1 <= first_length_modifier <= 1, f"Invalid first length modifier first_length_modifier={first_length_modifier}"
    assert -1 <= second_length_modifier <= 1, f"Invalid second length modifier second_length_modifier={second_length_modifier}"
    assert -1 <= third_length_modifier <= 1, f"Invalid third length modifier third_length_modifier={third_length_modifier}"

    from tesselations import TesselationPolygon, TESSELATION_LINE_SYMETRIC, TESSELATION_LINE_NORMAL, TESSELATION_LINE_FLIPPED

    # convex initial conditions for each pentagon
    AA_init = {
        "R1": 70, "R2": 150, "R4": 135, "R5": 120, "R6": 120, "R7": 150, "R8": 90, "R10": 90,
        "R11": 130, "R12": 125, "R13": 135,
    }.get(pentagon_type, 0)
    ...
    R1yoff = _sc(R1scale, [[-b * cos(AA) - e * cos(AA) + c * cos(AA + BB), b * sin(AA) + e * sin(AA) - c * sin(AA + BB)]])[0]
    ...
    R2_2 = TesselationPolygon(
        _sc(R2scale, [
            [0.5 * (b - 2 * cos(AA) + 2 * b * cos(AA - BB) - 2 * cos(BB)), -sin(AA) + b * sin(AA - BB) + sin(BB)],
            [0.5 * b + e, 0],
            [b / 2, 0],
            [0.5 * (b - 2 * cos(BB)), sin(BB)],
    ...
    R2xoff = _sc(R2scale, [[0.5 * b - cos(AA) + 0.5 * (b + 2 * b * cos(AA - BB) - 2 * cos(BB)), -sin(AA) + b * sin(AA - BB) + sin(BB)]])[0]
    R2yoff = _sc(R2scale, [[b + e - 2 * cos(AA) + (b - e) * cos(AA - BB), -2 * sin(AA) + (b - e) * sin(AA - BB)]])[0]
    ...
    R5_3 = _sc(R5scale, [
        ...
        [0.5 * (-1 + 3 * b * cos(AA) - sqrt(3) * b * sin(AA)), 0.5 * (sqrt(3) - sqrt(3) * b * cos(AA) - 3 * b * sin(AA))],
        [-1, 0],
    ])
    R5_4 = _sc(R5scale, [
        [0, 0],
    ...
    R5xoff = _sc(R5scale, [[0.5 * (3 - 3 * b * cos(AA) + sqrt(3) * b * sin(AA)), 0.5 * (-sqrt(3) + sqrt(3) * b * cos(AA) + 3 * b * sin(AA))]])[0]
    ...
    R6_2 = _sc(R6scale, [
        ...
        [-1 - cos(AA / 2) + cos(AA) + cos(3 * AA / 2), 0.5 * (-2 * cos(3 * AA / 4) + cos(7 * AA / 4)) * (1 / sin(AA / 4))],
        [-0.5 + cos(AA), -1 * sin(AA)],
        [-0.5, 0],
    ])
    R6_3 = _sc(R6scale, [
    ...
    R6yoff = _sc(R6scale, [[0.5 - cos(AA / 2) - 2 * cos(AA), 0.5 * cos(AA / 4) / sin(AA / 4) + sin(AA / 2) + 2 * sin(AA)]])[0]
    ...
    R7xoff = _sc(R7scale, [[1 + (2 * cos(AA)) / (-2 - cos(AA) + cos(3 * AA)), (sin(AA) - sin(3 * AA)) / (2 + cos(AA) - cos(3 * AA))]])[0]
    R7yoff = _sc(R7scale, [[
    ...
    R8 = _sc(R8scale, [
        ...
        [1 / 2, 0],
        [1 / 2 - cos(AA), -1 * sin(AA)],
        [-(3 / 4) + 9 / (4 * (5 + 4 * cos(AA / 2))) - cos(AA), (-5 * (sin(AA / 2) + sin(AA)) - 2 * sin((3 * AA) / 2)) / (5 + 4 * cos(AA / 2))],
        [-(1 / 2) - cos(AA / 2), -1 * sin(AA / 2)],
    ])
    R8_2 = _sc(R8scale, [
        [-((11 + 22 * cos(AA / 2) + 12 * cos(AA)) / (10 + 8 * cos(AA / 2))), -((3 * (3 * sin(AA / 2) + 2 * sin(AA))) / (5 + 4 * cos(AA / 2)))],
    ...
    R8_4 = _sc(R8scale, [
        [(11 + 22 * cos(AA / 2) + 12 * cos(AA)) / (10 + 8 * cos(AA / 2)), (9 * sin(AA / 2) + 6 * sin(AA)) / (5 + 4 * cos(AA / 2))],
        [(11 + 24 * cos(AA / 2) + 20 * cos(AA) + 8 * cos((3 * AA) / 2)) / (10 + 8 * cos(AA / 2)),
         (2 * (5 * (sin(AA / 2) + sin(AA)) + 2 * sin((3 * AA) / 2))) / (5 + 4 * cos(AA / 2))],
        [(3 + 14 * cos(AA / 2) + 20 * cos(AA) + 8 * cos((3 * AA) / 2)) / (10 + 8 * cos(AA / 2)),
         ((11 + 20 * cos(AA / 2) + 8 * cos(AA)) * sin(AA / 2)) / (5 + 4 * cos(AA / 2))],
    ...
    R9_5 = _sc(R9scale, [
        ...
        [-3 + 8 / (b * b) + (4 * (-7 + 3 * b * b)) / (8 - 5 * b * b + b * b * b * b), (4 * (-5 + b * b) * sqrt(-1 + b * b)) / (8 - 5 * b * b + b * b * b * b)],
    ...
    R9_6 = _sc(R9scale, [
        [-2 + 4 / (b * b) + (4 * (-1 + b * b)) / (8 - 5 * b * b + b * b * b * b),
         -((sqrt(-1 + b * b) * (32 - 5 * b * b * b * b + b * b * b * b * b * b)) / (b * b * (8 - 5 * b * b + b * b * b * b)))],
        [-1 + 4 / (b * b) + (8 * (-2 + b * b)) / (8 - 5 * b * b + b * b * b * b), (4 * (-8 + b * b) * sqrt(-1 + b * b)) / (b * b * (8 - 5 * b * b + b * b * b * b))],
        [-4 + 8 / (b * b) + (8 * (-2 + b * b)) / (8 - 5 * b * b + b * b * b * b),
         -((sqrt(-1 + b * b) * (24 - 9 * b * b + b * b * b * b)) / (8 - 5 * b * b + b * b * b * b))],
        [4 * (-1 + 1 / (b * b) + (-1 + b * b) / (8 - 5 * b * b + b * b * b * b)),
         -((sqrt(-1 + b * b) * (32 - 5 * b * b * b * b + b * b * b * b * b * b)) / (b * b * (8 - 5 * b * b + b * b * b * b)))],
        [-3 + 4 / (b * b) + (4 * (-1 + b * b)) / (8 - 5 * b * b + b * b * b * b), (8 * (-4 + b * b) * sqrt(-1 + b * b)) / (b * b * (8 - 5 * b * b + b * b * b * b))],
    ])
    R9_7 = _sc(R9scale, [
    ...
    R9_8 = _sc(R9scale, [
        [-1, 0],
        [-2 - (4 * (-3 + b * b)) / (8 - 5 * b * b + b * b * b * b), -((sqrt(-1 + b * b) * (4 - 5 * b * b + b * b * b * b)) / (8 - 5 * b * b + b * b * b * b))],
        [1 - 4 / (b * b) - (4 * (-3 + b * b)) / (8 - 5 * b * b + b * b * b * b),
         -((4 * sqrt(-1 + b * b) * (8 - 6 * b * b + b * b * b * b)) / (b * b * (8 - 5 * b * b + b * b * b * b)))],
        [1, 0],
        [0, -1 * sqrt(-1 + b * b)],
    ])
    R9xoff = _sc(R9scale, [[(4 * (-1 + b * b)) / (8 - 5 * b * b + b * b * b * b), (4 * (-3 + b * b) * sqrt(-1 + b * b)) / (8 - 5 * b * b + b * b * b * b)]])[0]
    R9yoff = _sc(R9scale, [[
    ...
    R10_2 = _sc(R10scale, [
        [0, 1],
        [(-2 + 2 * cos(AA) + sin(AA)) / (1 + sin(AA / 2) / cos(AA / 2)), (1 / 2) * (-1 + cos(AA) + 3 * sin(AA))],
    ...
    R10_5 = _sc(R10scale, [
        [(1 / 2) * (-3 + cos(AA) + 6 / (1 + cos(AA / 2) / sin(AA / 2)) + 3 * sin(AA)), -((cos(AA / 2) * (-2 + 2 * cos(AA) + sin(AA))) / (cos(AA / 2) + sin(AA / 2)))],
        [(1 / 2) * (-1 + cos(AA) + 3 * sin(AA)), -((-2 + 2 * cos(AA) + sin(AA)) / (1 + sin(AA / 2) / cos(AA / 2)))],
        [(-1 + cos(AA) + 2 * sin(AA)) / (1 + sin(AA / 2) / cos(AA / 2)), (1 / 2) * (-1 - cos(AA) + 3 * sin(AA))],
        [(cos(AA / 2) * (-1 + cos(AA) + 2 * sin(AA))) / (cos(AA / 2) + sin(AA / 2)), (cos(AA) + cos(AA / 2) / sin(AA / 2) + 2 * sin(AA)) / (1 + cos(AA / 2) / sin(AA / 2))],
    ...
    R12 = _sc(R12scale, [
        ...
        [(4 * cos(AA / 4) + cos((3 * AA) / 4) + cos((5 * AA) / 4)) * (1 / sin(3 * AA / 4)), 1 + (1 + cos(AA) + cos((3 * AA) / 2)) / (-cos(AA / 2) + cos(AA))],
    ...
    R12_2 = _sc(R12scale, [
        [0, 1],
        [3 * (cos(AA / 4) / sin(AA / 4)) - 4 * sin(AA / 2), 1],
        [(4 * cos(AA / 4) + cos((3 * AA) / 4) + cos((5 * AA) / 4)) * (1 / sin(3 * AA / 4)), (1 + cos(AA / 2) + cos((3 * AA) / 2)) / (cos(AA / 2) - cos(AA))],
        [1 / 2 * (-(4 / (1 + 2 * cos(AA / 2))) + (1 / sin(AA / 4)) * (1 / sin(AA / 4))) * sin(AA), ((-1 + 4 * cos(AA / 2)) * cos(AA)) / (cos(AA / 2) - cos(AA))],
        [0, 0],
    ])
    R12_3 = _sc(R12scale, [
        [(8 * cos(AA / 4) + 4 * cos((3 * AA) / 4) + 3 * cos((5 * AA) / 4)) * (1 / sin(3 * AA / 4)), 1 + (1 + cos(AA) + cos((3 * AA) / 2)) / (cos(AA / 2) - cos(AA))],
        [(4 * cos(AA / 4) + cos((3 * AA) / 4) + cos((5 * AA) / 4)) * (1 / sin(3 * AA / 4)), 1 + (1 + cos(AA) + cos((3 * AA) / 2)) / (cos(AA / 2) - cos(AA))],
        [3 * (cos(AA / 4) / sin(AA / 4)) - 4 * sin(AA / 2), 1 + (2 * (1 + cos(AA) + cos((3 * AA) / 2))) / (cos(AA / 2) - cos(AA))],
        [3 * (cos(AA / 4) / sin(AA / 4)) - 2 * sin(AA / 2), (1 + 2 * cos(AA / 2) + 3 * cos((3 * AA) / 2)) / (cos(AA / 2) - cos(AA))],
        [(8 * cos(AA / 4) + 4 * cos((3 * AA) / 4) + 3 * cos((5 * AA) / 4)) * (1 / sin(3 * AA / 4)), (1 + cos(AA) + cos((3 * AA) / 2)) / (cos(AA / 2) - cos(AA))],
    ])
    R12_4 = _sc(R12scale, [
        [(8 * cos(AA / 4) + 4 * cos((3 * AA) / 4) + 3 * cos((5 * AA) / 4)) * (1 / sin(3 * AA / 4)), 1 + (1 + cos(AA) + cos((3 * AA) / 2)) / (cos(AA / 2) - cos(AA))],
        [(4 * cos(AA / 4) + cos((3 * AA) / 4) + cos((5 * AA) / 4)) * (1 / sin(3 * AA / 4)), 1 + (1 + cos(AA) + cos((3 * AA) / 2)) / (cos(AA / 2) - cos(AA))],
        ...
        [(8 * cos(AA / 4) + 4 * cos((3 * AA) / 4) + 3 * cos((5 * AA) / 4)) * (1 / sin(3 * AA / 4)), 1 + (1 + cos(AA / 2) + cos((3 * AA) / 2)) / (cos(AA / 2) - cos(AA))],
    ])
    R12_5 = _sc(R12scale, [
        [(1 / 2) * (-3 * cos(AA / 4) - 4 * cos((3 * AA) / 4) + cos((5 * AA) / 4)) * (1 / sin(3 * AA / 4)), 2 + 1 / (-1 - 2 * cos(AA / 2)) + 1 / (-1 + cos(AA / 2)) + cos(AA / 2)],
        [(2 * cos(AA / 4) + cos((3 * AA) / 4) + 2 * cos((5 * AA) / 4) + cos((7 * AA) / 4)) * (1 / sin(3 * AA / 4)), (1 - cos(AA / 2) + cos(AA) + cos((3 * AA) / 2) + cos(2 * AA)) / (-cos(AA / 2) + cos(AA))],
        [0, 2],
        [0, 0],
        [(-cos(AA / 4) - 2 * cos((3 * AA) / 4)) * (1 / sin(3 * AA / 4)), 2 + (2 + cos(AA / 2)) / (-cos(AA / 2) + cos(AA))],
    ])
    R12_6 = _sc(R12scale, [
        [(1 / 2) * (-3 * cos(AA / 4) - 4 * cos((3 * AA) / 4) + cos((5 * AA) / 4)) * (1 / sin(3 * AA / 4)), 2 + 1 / (-1 - 2 * cos(AA / 2)) + 1 / (-1 + cos(AA / 2)) + cos(AA / 2)],
        [(2 * cos(AA / 4) + cos((3 * AA) / 4) + 2 * cos((5 * AA) / 4) + cos((7 * AA) / 4)) * (1 / sin(3 * AA / 4)), (1 - cos(AA / 2) + cos(AA) + cos((3 * AA) / 2) + cos(2 * AA)) / (-cos(AA / 2) + cos(AA))],
        [(2 * cos(AA / 4) + cos((3 * AA) / 4) + 2 * cos((5 * AA) / 4) + cos((7 * AA) / 4)) * (1 / sin(3 * AA / 4)), (cos(AA / 2) + 3 * cos(AA) + cos((3 * AA) / 2) + cos(2 * AA)) / (-cos(AA / 2) + cos(AA))],
        [(cos(AA / 4) + cos((3 * AA) / 4) + 2 * (cos((5 * AA) / 4) + cos((7 * AA) / 4))) * (1 / sin(3 * AA / 4)), (1 + 3 * cos(AA) + 2 * cos(2 * AA)) / (-cos(AA / 2) + cos(AA))],
        [(-2 * (cos(AA / 4) + cos((3 * AA) / 4)) + cos((5 * AA) / 4)) * (1 / sin(3 * AA / 4)), (1 + cos(AA) + cos((3 * AA) / 2)) / (-cos(AA / 2) + cos(AA))],
    ])
    R12_7 = _sc(R12scale, [
        ...
        [(2 * cos(AA / 4) + cos((3 * AA) / 4) + 2 * cos((5 * AA) / 4) + cos((7 * AA) / 4)) * (1 / sin(3 * AA / 4)), (cos(AA / 2) + 3 * cos(AA) + cos((3 * AA) / 2) + cos(2 * AA)) / (-cos(AA / 2) + cos(AA))],
        [2 * (2 * cos(AA / 4) + cos((3 * AA) / 4) + 2 * cos((5 * AA) / 4) + cos((7 * AA) / 4)) * (1 / sin(3 * AA / 4)), -((1 / 2)) * (1 + 2 * cos((3 * AA) / 2)) * (1 / sin(AA / 4)) * (1 / sin(AA / 4))],
        [2 * (2 * cos(AA / 4) + cos((3 * AA) / 4) + 2 * cos((5 * AA) / 4) + cos((7 * AA) / 4)) * (1 / sin(3 * AA / 4)), 8 + 8 * cos(AA / 2) + 4 * cos(AA) - 3 / 2 * (1 / sin(AA / 4)) * (1 / sin(AA / 4))],
    ...
    R12_8 = _sc(R12scale, [
        ...
        [(2 * cos(AA / 4) + cos((3 * AA) / 4) + 2 * cos((5 * AA) / 4) + cos((7 * AA) / 4)) * (1 / sin(3 * AA / 4)), (cos(AA / 2) + 3 * cos(AA) + cos((3 * AA) / 2) + cos(2 * AA)) / (-cos(AA / 2) + cos(AA))],
        [(2 * cos(AA / 4) + cos((3 * AA) / 4) + 2 * cos((5 * AA) / 4) + cos((7 * AA) / 4)) * (1 / sin(3 * AA / 4)), (1 - cos(AA / 2) + cos(AA) + cos((3 * AA) / 2) + cos(2 * AA)) / (-cos(AA / 2) + cos(AA))],
        [(3 * cos(AA / 4) + cos((3 * AA) / 4) + 2 * cos((5 * AA) / 4)) * (1 / sin(3 * AA / 4)), (cos(AA) + 2 * cos((3 * AA) / 2)) / (-cos(AA / 2) + cos(AA))],
        [(6 * cos(AA / 4) + 4 * cos((3 * AA) / 4) + 3 * cos((5 * AA) / 4) + 2 * cos((7 * AA) / 4)) * (1 / sin(3 * AA / 4)), (3 * cos(AA) + cos((3 * AA) / 2) + 2 * cos(2 * AA)) / (-cos(AA / 2) + cos(AA))],
    ])
    R12xoff = _sc(R12scale, [[
    ...
    patterns = {
        ...
        "R15": [R15, [R15, R15_2, R15_3, R15_4, R15_5, R15_6, R15_7, R15_8, R15_9, R15_10, R15_11, R15_12], R15xoff, R15yoff],
    }
    pattern = patterns.get(pentagon_type, [[], [], [0, 0], [0, 0]])

    points = _bosl2.make_region(InnerPentagonTesselation(pattern=pattern, pentagon_size=pentagon_size, thickness=thickness))
    return types.SimpleNamespace(y_offset=pattern[2], x_offset=pattern[3], points=points)


def pentagonBorder(vertices: list[list[float]], size: float, thickness: float) -> list[list[float]]:
    """Internal: builds the outline region for one pentagon instance."""
    scaled = comparisons.deduplicate(paths.path_merge_collinear(_sc(size, vertices), closed=True), closed=True)
    outer = paths.path_merge_collinear(_bosl2.offset(path=scaled, delta=0.01, closed=True), closed=True)
    inner = paths.path_merge_collinear(_bosl2.offset(path=scaled, delta=-1 * thickness, closed=True), closed=True)
    return _bosl2.difference(_bosl2.force_region(outer), _bosl2.force_region(inner))


def InnerPentagonTesselation(pattern: list, pentagon_size: float, thickness: float) -> list[list[float]]:
    """Internal: builds the region data for all pentagon instances in a pattern."""
