...
2. **PythonSCAD ports** — each `foo.scad` has a `foo.py` port that runs inside the PythonSCAD app's embedded Python. These build on `base_bgtk.py` (which loads real BOSL2 via `osuse("BOSL2/std.scad")` at import time) and on `bosl2/` (a pure-Python numpy port of the BOSL2 pieces the toolkit needs), with shared lid machinery in `lids_base.py` and decorative parts in `components.py`, `labels.py`, `shapes.py`, `tesselations*`.
   ...
   **Every box type is a `BoxBaseType` subclass** (`box_base.py`) built from one frozen `BoxSpec`, with two public methods: `make_box()` and `make_lid()`. `box_base.py` owns both pipelines; a subclass supplies geometry only, through at most three hooks:
   ...
   - `_lid_plate(lid) -> LidPlate` — the flat face the decoration goes on, plus `shell` / `offset` / `origin` / `path` / `cutouts`. There is ONE lid pipeline: `make_lid` fits the `Lid`'s pattern + label + fingernail to that plate via `build_lid`. Do not assemble overlays in a box type. `_lid_adjustment(stack)` is the print-orientation hook.

     **One field says what a lid looks like**: `BoxSpec.lid`, which is `None` (plain), a `str` (that text as a label), or a `Lid` (pattern, label styling, fingernail, extras). It replaced five fields — `lid`/`lid_label`/`lid_shape`/`label_options`/`shape_options` — whose legal combinations had to be policed in `__post_init__` and re-resolved at build time. Do not add a second lid field; put it on the `Lid`. A `BoxKit` shares one `Lid` across a project and each box overrides with a `str`, which **merges** via `Lid.with_label()` rather than replacing — that merge is the only reason one field is as convenient as five, and it is covered by `tests/test_box_spec_lid.py` (pure Python, in the fast suite).
...
The `bosl2/` port wraps native solids in `Bosl2Solid`, which carries optional cuboid `size`/`anchor` metadata AND a bbox-backed anchoring layer: `bounds()`/`anchor_point()`/`reanchor()`/`position()`/`attach()`/`align()` read PythonSCAD's native `obj.position`/`obj.size`/`obj.bbox` (the AABB, computed by meshing) so anchoring/attachment/edge-masking work on ANY object without threading a size through the calls. `bounds()` prefers the native bbox (always correct after transforms/CSG), falling back to tracked metadata only under the numeric mock. Direct-CSG shapes are strongly preferred over SDF solids (see the SDF-vs-direct note below). **No box builds a hinge from SDF any more.** `filament_hinge_box` and `card_library` both moved to the CSG `pybosl2.parts.KnuckleHinge` at pybosl2 0.7.8, which fixed leaves that intersected at every fold angle (a hinge that printed as one fused solid and could not open) -- 18602 facets to 796 and 18708 to 870 respectively, via the shared `components.knuckle_hinge_leaf()`. What legitimately REMAINS on SDF is what has no CSG equivalent: `inset_box`'s `rabbit_clip` and `shapes3d`'s `convex_polyhedron`. That is the bar for reaching for SDF.
...
- **No wildcard imports.** Every `from X import *` in the toolkit is gone (they were 71, across 40 files). `base_bgtk` used to `from pythonscad import *`, which re-exported all 101 native builtins under its own name, so the 30 files doing `from base_bgtk import *` silently pulled in `cube`/`sphere`/`union`/… and no import line said which of the two a name came from. `base_bgtk.__all__` now pins its prelude surface at 60 names and deliberately excludes the stdlib it happens to import (`os`, `math`, `np`, `dataclass`, …). The whole repo uses just **8 native free functions** — `circle`, `cube`, `minkowski`, `polygon`, `sphere`, `square`, `text`, `textmetrics` — at 16 sites, each now explicit; that list IS the remaining native surface to migrate to pybosl2. The ONE legitimate exception is the render harnesses in `tests/`, which embed `from base_bgtk import *` inside script *strings*: those are an eval namespace for arbitrary test expressions. Beware when bulk-editing imports — a wildcard inside a triple-quoted script starts at column 0 and looks exactly like a real one.
- **There are no type stubs.** `typings/*.pyi` is gone. `openscad` does `from _openscad import *` out of a C extension, so pyright cannot see ANY native name: `from pythonscad import cube` and the 30 `if TYPE_CHECKING: from openscad import PyOpenSCAD` lines each report `unknown import symbol`. That is ~48 of the repo's pyright errors and is infrastructure, not a code defect — compare pyright totals before/after a change rather than expecting zero.
