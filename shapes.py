...
def sword2d_outline(
    ...
    outline = DifferenceWithOffset(
        ...
        children=sword2d(
            ...
            hilt_length=calc_hilt_length,
            hilt_pos=calc_hilt_pos,
            hilt_rounding=calc_hilt_rounding,
            blade_rounding=blade_rounding,
        ),
    )
...
def half_eye2d(angle: float, outer_size: float = 10, inner_size: float = 8, pupil_size: float = 5) -> "Bosl2Shape2D":
    ...
    cutter = (
        ...
        .rotate([0, 0, angle])
        .translate([outer_size * _cosd(angle), outer_size * _sind(angle)])
    )
    eye = eye - cutter
...
def portugal_castle(width: float) -> "Bosl2Shape2D":
    """The castle from the flag of Portugal.

    Usage::

        portugal_castle(20)

    Loaded from ``svg/portugal_castle.svg``, which is what the castle actually is: FILLED
    shapes (twelve yellow, three black). This used to be 469 lines of traced bezier control
    points run through ``_stroke()`` with a ``stroke_width`` -- an OUTLINE of the castle
    rather than the castle -- and the flag imported it without ever calling it.

    Args:
        width: the final width of the castle; the height follows the drawing's aspect.
    """
    region = Region.from_svg(str(_SVG_DIR / "portugal_castle.svg"))
    # The drawing's own extents, so the aspect and the centring both come from the file
    # rather than from numbers that have to be kept in step with it by hand.
    min_x, min_y, max_x, max_y = region.geom.bounds
    return (
        region.geometry()
        # Centred on the origin, as the traced version was.
        .translate([-(min_x + max_x) / 2, -(min_y + max_y) / 2])
        .resize([width, width * (max_y - min_y) / (max_x - min_x), 0])
    )
