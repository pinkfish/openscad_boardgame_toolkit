...
def MakeShapeObject(
    shape_type: ShapeType = default_lid_shape_type,
    shape_width: float = default_lid_shape_width,
    shape_thickness: float = default_lid_shape_thickness,
    shape_aspect_ratio: float = default_lid_aspect_ratio,
    rounding: float = default_lid_shape_rounding,
    supershape_m1: float = default_lid_supershape_m1,
    supershape_m2: float = default_lid_supershape_m2,
    supershape_n1: float = default_lid_supershape_n1,
    supershape_n2: float = default_lid_supershape_n2,
    supershape_n3: float = default_lid_supershape_n3,
    supershape_a: float = default_lid_supershape_a,
    supershape_b: float = default_lid_supershape_b,
    ...
    """Builds a shape options namespace for use with ShapeByType().

    Args:
        shape_type: The type of shape to use
        shape_width: width of the shape
        shape_thickness: thickness of the shape
        shape_aspect_ratio: aspect ratio of the shape
        rounding: rounding of the shape
        supershape_m1/supershape_m2/supershape_n1/supershape_n2/supershape_n3/supershape_a/supershape_b:
            supershape parameters
        pentagon_first_angle_modifier/pentagon_second_angle_modifier/pentagon_first_length_modifier/
        pentagon_second_length_modifier/pentagon_third_length_modifier: pentagon tiling modifiers
    """
...
def Sword2d(
    ...
    Args:
        ...
        blade_rounding: how much rounding on the end of the blade (default width / 8)
        hilt_rounding: how much rounding on the hilt (default length / 40)
    """
    calc_hilt_length = hilt_length
    if calc_hilt_length is None:
        calc_hilt_length = length / 15
    calc_hilt_pos = hilt_pos
    if calc_hilt_pos is None:
        calc_hilt_pos = length / 7
    calc_blade_width = blade_width
    if calc_blade_width is None:
        calc_blade_width = width / 3
    calc_hilt_rounding = hilt_rounding
    if calc_hilt_rounding is None:
        calc_hilt_rounding = length / 40
    # Original SCAD read hilt_rounding here instead of blade_rounding, leaving
    # blade_rounding completely unused -- fixed to read the right parameter.
    calc_blade_rounding = blade_rounding
    if calc_blade_rounding is None:
        calc_blade_rounding = width / 8

    blade = shapes2d.rect([length, calc_blade_width], rounding=[0, calc_blade_rounding, calc_blade_rounding, 0])
    blade = blade - shapes2d.right_triangle([length / 3, width / 2], spin=180).translate([length / 2, width / 2])
    blade = blade - shapes2d.right_triangle([length / 3, width / 2], spin=180).translate(
        [length / 2, width / 2]
    ).mirror([0, 1])
    hilt = shapes2d.rect([calc_hilt_length, width], rounding=calc_hilt_rounding).translate(
        [-length / 2 + calc_hilt_pos + calc_hilt_length / 2, 0]
    )
    return blade | hilt
...
def Sword2dOutline(
    ...
    Args:
        ...
        hilt_length: length of the hilt (default length / 15)
        hilt_pos: position on the blade where the hilt starts (default length / 7)
        blade_rounding: how much rounding on the end of the blade
        hilt_rounding: how much rounding on the hilt
    ...
    hilt_outline = DifferenceWithOffset(
        offset=-line_width, children=shapes2d.rect([calc_hilt_length, width], rounding=calc_hilt_rounding)
    ).translate([-length / 2 + calc_hilt_pos + calc_hilt_length / 2, 0])

    strap = shapes2d.rect([length - calc_hilt_pos - calc_hilt_length, line_width]).translate(
        [-length / 2 + calc_hilt_pos + (length - calc_hilt_pos - calc_hilt_length) / 2 + calc_hilt_length, 0]
    )
    strap = strap - shapes2d.right_triangle([length / 3, width / 2], spin=180).translate([length / 2, width / 2])
    strap = strap - shapes2d.right_triangle([length / 3, width / 2], spin=180).translate(
        [length / 2, width / 2]
    ).mirror([0, 1])
...
def Crossbow2d(
    length: float, width: float, handle_width: float | None = None, bow_width: float | None = None, outer_circle: float | None = None
) -> PyOpenSCAD:
    """A 2d crossbow shape.
    ...
    """
    calc_handle_width = handle_width
    if calc_handle_width is None:
        calc_handle_width = width / 8
    calc_outer_circle = outer_circle
    if calc_outer_circle is None:
        calc_outer_circle = width * 2
    calc_bow_width = bow_width
    if calc_bow_width is None:
    ...
    bound = shapes2d.rect([width, length])
...
def Crossbow2dOutline(
    ...
    outline = DifferenceWithOffset(
        offset=-line_width,
        children=Crossbow2d(length=length, width=width, handle_width=calc_handle_width, bow_width=bow_width, outer_circle=outer_circle),
    )
    handle_outline = DifferenceWithOffset(offset=-line_width, children=shapes2d.rect([calc_handle_width, length]))
...
def Sledgehammer2d(
    ...
    Args:
        ...
        width: width of the sledgehammer
        handle_width: width of the handle bit (default width/4)
        rounding_head: rounding amount on the head (default width/5)
        rounding_handle: rounding amount on the handle (default width/10)
        head_length: length of the head (default length/3.5)
    """
    calc_hammer_handle_width = handle_width
    if calc_hammer_handle_width is None:
        calc_hammer_handle_width = width / 4
    calc_hammer_head_length = head_length
    if calc_hammer_head_length is None:
        calc_hammer_head_length = length / 3.5
    calc_rounding_handle = rounding_handle
    if calc_rounding_handle is None:
        calc_rounding_handle = width / 10
    calc_rounding_head = rounding_head
    if calc_rounding_head is None:
        calc_rounding_head = width / 5
    handle = shapes2d.rect([calc_hammer_handle_width, length], rounding=calc_rounding_handle)
    head = shapes2d.rect([width, calc_hammer_head_length], rounding=calc_rounding_head).translate(
        [0, length / 2 - calc_hammer_head_length / 2]
    )
    return handle | head
...
def Sledgehammer2dOutline(
    ...
    Args:
        ...
        handle_width: width of the handle bit (default width/4)
        head_length: length of the head (default length/3.5)
        rounding_head: rounding amount on the head (default 2)
    ...
    if calc_hammer_head_length is None:
        calc_hammer_head_length = length / 3.5
    calc_strap_width = strap_width
    if calc_strap_width is None:
        calc_strap_width = line_width * 4

    body_outline = DifferenceWithOffset(
        offset=-line_width,
        children=Sledgehammer2d(length=length, width=width, handle_width=calc_hammer_handle_width, head_length=calc_hammer_head_length),
    )
    head_outline = DifferenceWithOffset(
        offset=-line_width, children=shapes2d.rect([width, calc_hammer_head_length], rounding=rounding_head)
...
def Shoe2d(
    ...
    """
    calc_leg_length = leg_length
    if calc_leg_length is None:
        calc_leg_length = size / 3
    calc_base_width = base_width
    if calc_base_width is None:
        calc_base_width = size / 3
    calc_sole_height = sole_height
    if calc_sole_height is None:
        calc_sole_height = size / 20

    leg = shapes2d.rect([size, calc_leg_length]).translate([0, -size / 2 + calc_leg_length / 2])
...
def Bag2d(
    ...
    """
    calc_base_round = base_round_diameter
    if calc_base_round is None:
        calc_base_round = size / 4
    calc_main_round = main_round_diameter
    if calc_main_round is None:
        calc_main_round = size / 2
    calc_neck_width = neck_width
    if calc_neck_width is None:
        calc_neck_width = size / 6
    calc_rope_length = rope_length
    if calc_rope_length is None:
...
def Torch2d(length: float, width: float, handle_width: float | None = None, head_length: float | None = None) -> PyOpenSCAD:
    ...
    calc_handle_width = handle_width
    if calc_handle_width is None:
        calc_handle_width = width / 2
    calc_head_length = head_length
...
def Torch2dOutline(
    length: float, width: float, handle_width: float | None = None, head_length: float | None = None, line_width: float = 1
) -> PyOpenSCAD:
    ...
    if calc_head_length is None:
        calc_head_length = length / 7
    outline = DifferenceWithOffset(
        offset=-line_width, children=Torch2d(length=length, width=width, handle_width=calc_handle_width, head_length=calc_head_length)
    )
    head_hull = hull(
        circle(d=width / 2).translate([length / 2 - width / 4, width / 4]),
        circle(d=width / 2).translate([length / 2 - width / 4 - calc_head_length, width / 4]),
...
def Teapot2d(
    ...
    top_circle_diameter: float | None = None,
    handle_rounding: float | None = None,
    handle_width: float | None = None,
    ...
    Args:
        ...
        handle_rounding: rounding amount to use in the top handle
    ...
    """
    calc_top_width = top_width
    if calc_top_width is None:
        calc_top_width = width * 6 / 10
    calc_top_length = top_length
    if calc_top_length is None:
        calc_top_length = length * 7 / 10
    calc_top_circle_diameter = top_circle_diameter
    if calc_top_circle_diameter is None:
        calc_top_circle_diameter = calc_top_width * 8 / 10
    calc_handle_rounding = handle_rounding
    if calc_handle_rounding is None:
        calc_handle_rounding = width / 15
    calc_handle_width = handle_width
    if calc_handle_width is None:
        calc_handle_width = width / 30
    calc_spout_length = spout_length
    if calc_spout_length is None:
        calc_spout_length = length / 5
    calc_side_handle_length = side_handle_length
    if calc_side_handle_length is None:
        calc_side_handle_length = length / 3
    calc_side_handle_rounding = side_handle_rounding
    if calc_side_handle_rounding is None:
    ...
    # Handle
    handle = DifferenceWithOffset(
        offset=-calc_handle_width, children=shapes2d.rect([diff_top * 2, diff_top * 2], rounding=calc_handle_rounding)
...
def CoinPile2d(size: float, rounding: float | None = None, coin_num: int | None = None) -> PyOpenSCAD:
    ...
    """
    calc_coin_num = coin_num
    if calc_coin_num is None:
        calc_coin_num = 5
    calc_rounding = rounding
    if calc_rounding is None:
        calc_rounding = (size * 3 / 4) / (calc_coin_num * 2)
...
def Rock2d(length: float, width: float, rounding: float | None = None) -> PyOpenSCAD:
    ...
    Args:
        ...
        rounding: rounding on the edge of the rock (unused, kept for source parity)
    """
    calc_rounding = min(length, width) / 5
    return shapes2d.rect([length, width], rounding=calc_rounding)


# Constant: ruins_2d_length - the length of the ruins svg
ruins_2d_length = 100
# Constant: ruins_2d_width - the length of the ruins svg
...
def RockWall2d(size: float, num_rows: int = 10, num_cols: int = 40, spacing: float | None = None) -> PyOpenSCAD:
    ...
    Args:
        ...
        num_cols: number of cols (default 40)
...
def Anvil2d(size: float, with_hammer: bool = False) -> PyOpenSCAD:
    ...
    """
    new_height = size if with_hammer else size * 10 / 18

    shape = shapes2d.rect([10, 1])
...
def SingleLog2d(size: float, line_width: float = 1) -> PyOpenSCAD:
    ...
    d = DifferenceWithOffset(
        offset=-line_width, children=hull(circle(d=size), circle(d=size).translate([20, 10]))
    )
...
def Sign2d(size: float) -> PyOpenSCAD:
    ...
    """
    post = shapes2d.rect([size / 10, size * 3 / 4]) - shapes2d.rect([size, size / 2]).translate(
        [0, size * 1.5 / 4 - size / 4 - size / 20]
    )
    board = DifferenceWithOffset(
        -1, children=shapes2d.rect([size, size / 2]).translate([0, size * 1.5 / 4 - size / 4 - size / 20])
    )
    return post | board
...
def PortugalCastle(stroke_width: float, width: float) -> PyOpenSCAD:
    ...
    shape = None
    piece0 = _stroke(Bezier([[190.19, 154.43], [190.32493, 148.90900000000002], [194.2424, 147.602], [194.2706, 147.5826], [194.2988, 147.56410000000002], [198.502, 148.99020000000002], [198.4879, 154.4812], [198.4879, 154.4812], [190.1901, 154.43], [190.1901, 154.43]]).path_curve(), width=stroke_width).color("black")
    shape = piece0 if shape is None else shape | piece0
    piece1 = _stroke(Bezier([[186.81, 147.69], [186.81, 147.69], [186.12828, 154.0347], [186.12828, 154.0347], [186.12828, 154.0347], [190.26888, 154.04369999999997], [190.26888, 154.04369999999997], [190.30858, 148.79439999999997], [194.24277999999998, 147.92119999999997], [194.33798, 147.94059999999996], [194.42708, 147.93559999999997], [198.32688, 149.10119999999995], [198.43088, 154.04369999999997], [198.43088, 154.04369999999997], [202.58198, 154.04369999999997], [202.58198, 154.04369999999997], [202.58198, 154.04369999999997], [201.83236, 147.65049999999997], [201.83236, 147.65049999999997], [201.83236, 147.65049999999997], [186.81036, 147.68839999999997], [186.81036, 147.68839999999997], [186.81036, 147.68839999999997], [186.81036, 147.69039999999998], [186.81036, 147.69039999999998]]).path_curve(), width=stroke_width).color("black")
    shape = piece1 if shape is None else shape | piece1
    piece2 = _stroke(Bezier([[185.85, 154.06], [185.85, 154.06], [202.796, 154.06], [202.796, 154.06], [203.15317, 154.06], [203.44508, 154.41277], [203.44508, 154.84404], [203.44508, 155.27443], [203.15317, 155.62545], [202.796, 155.62545], [202.796, 155.62545], [185.85, 155.62545], [185.85, 155.62545], [185.49283, 155.62545], [185.20092, 155.27443], [185.20092, 154.84404], [185.20092, 154.41277], [185.49283, 154.06], [185.85, 154.06]]).path_curve(), width=stroke_width).color("black")
    shape = piece2 if shape is None else shape | piece2
    piece7 = _stroke(Bezier([[196.19, 138.57], [196.19, 138.57], [196.46690999999998, 145.0214], [196.46690999999998, 145.0214], [196.46690999999998, 145.0214], [192.16411, 145.0055], [192.16411, 145.0055], [192.16411, 145.0055], [192.44897, 138.5532], [192.44897, 138.5532], [192.44897, 138.5532], [196.18997000000002, 138.57], [196.18997000000002, 138.57]]).path_curve(), width=stroke_width).color("black")
    shape = piece7 if shape is None else shape | piece7
    piece8 = _stroke(Bezier([[190.94, 141.56], [190.94, 141.56], [191.07141, 145.0375], [191.07141, 145.0375], [191.07141, 145.0375], [186.94581, 145.0395], [186.94581, 145.0395], [186.94581, 145.0395], [187.06222, 141.5602], [187.06222, 141.5602], [187.06222, 141.5602], [190.94082, 141.5602], [190.94082, 141.5602], [190.94082, 141.5602], [190.93993, 141.5602], [190.93993, 141.5602]]).path_curve(), width=stroke_width).color("black")
    shape = piece8 if shape is None else shape | piece8
    piece9 = _stroke(Bezier([[186.3, 139.04], [186.3, 139.04], [187.4994, 139.04299999999998], [187.4994, 139.04299999999998], [187.4994, 139.04299999999998], [187.4994, 139.91523999999998], [187.4994, 139.91523999999998], [187.4994, 139.91523999999998], [188.3769, 139.91523999999998], [188.3769, 139.91523999999998], [188.3769, 139.91523999999998], [188.3769, 139.02271], [188.3769, 139.02271], [188.3769, 139.02271], [189.6063, 139.02670999999998], [189.6063, 139.02670999999998], [189.6063, 139.02670999999998], [189.6063, 139.91571], [189.6063, 139.91571], [189.6063, 139.91571], [190.48556, 139.91571], [190.48556, 139.91571], [190.48556, 139.91571], [190.48556, 139.02318], [190.48556, 139.02318], [190.48556, 139.02318], [191.71576, 139.02518], [191.71576, 139.02518], [191.71576, 139.02518], [191.71375999999998, 141.03688], [191.71375999999998, 141.03688], [191.71375999999998, 141.35085999999998], [191.46505999999997, 141.55546999999999], [191.17755999999997, 141.55546999999999], [191.17755999999997, 141.55546999999999], [186.86065999999997, 141.55546999999999], [186.86065999999997, 141.55546999999999], [186.57139999999995, 141.55546999999999], [186.30241999999996, 141.31999], [186.30152999999996, 141.02982999999998], [186.30152999999996, 141.02982999999998], [186.29852999999997, 139.04102999999998], [186.29852999999997, 139.04102999999998], [186.29852999999997, 139.04102999999998], [186.29940999999997, 139.04102999999998], [186.29940999999997, 139.04102999999998]]).path_curve(), width=stroke_width).color("black")
    shape = piece9 if shape is None else shape | piece9
    piece10 = _stroke(Bezier([[193.9, 140.61], [193.8735, 139.98294], [194.77661, 139.97589000000002], [194.76603, 140.61], [194.76603, 140.61], [194.76603, 142.1464], [194.76603, 142.1464], [194.76603, 142.1464], [193.90003, 142.1464], [193.90003, 142.1464], [193.90003, 142.1464], [193.90003, 140.6104], [193.90003, 140.6104]]).path_curve(), width=stroke_width).color("black")
    shape = piece10 if shape is None else shape | piece10
