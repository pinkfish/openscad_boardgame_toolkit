...
module MakeMainLidLabelSolid(width, length, lid_thickness, label, border = 2, offset = 4, font = default_label_font,
                             radius = 5, full_height = false, label_colour = undef,
                             material_colour = default_material_colour, background_colour = undef,
                             solid_background = false, label_background_colour = undef)
{
    ...
    color(material_colour)
        cuboid(size =
                   [
                       width, length, full_height ? lid_thickness - default_slicing_layer_height : lid_thickness / 2 -
                       default_slicing_layer_height
                   ],
    ...
    {
        color(material_colour)
            cuboid(size = [ width, length, full_height || border > 0 ? lid_thickness : lid_thickness / 2 ],
                   rounding = radius, edges = "Z", anchor = FRONT + LEFT + BOTTOM);
    ...
    ]) difference()
    {
        translate([ border + 0.01, border + 0.01, 0 ]) color(calc_background_colour)
            cuboid(size = [ width - border * 2 - 0.02, length - border * 2 - 0.02, default_slicing_layer_height ],
                   rounding = radius, edges = "Z", anchor = FRONT + LEFT + BOTTOM);
...
//   MakeMainLidLabelStriped(width = 20, length = 80, lid_thickness = 2,label = "Australia",
//   full_height = true, label_colour = "blue");
module MakeMainLidLabelStriped(width, length, lid_thickness, label, border = 2, offset = 4, font = default_label_font,
                               radius = 5, full_height = false, label_colour = undef,
                               material_colour = default_material_colour, background_colour = undef,
                               solid_background = false, label_background_colour = undef)
{
    ...
    translate([
        0, 0, full_height ? lid_thickness - default_slicing_layer_height : lid_thickness / 2 -
        default_slicing_layer_height
    ]) color(label_colour) TextShape(text_height = default_slicing_layer_height * 2, edge_offset = 0.01);
    difference()
    {
        StripedBackground();
        translate([
            0, 0, full_height ? lid_thickness - default_slicing_layer_height : lid_thickness / 2 -
            default_slicing_layer_height
        ]) color(material_colour) TextShape(text_height = default_slicing_layer_height * 2, edge_offset = 0.01);
    }
}
                        ...
                        label_background_colour = undef)
{
    if (solid_background)
    {
        MakeMainLidLabelSolid(width = width, length = length, lid_thickness = lid_thickness, label = label,
    ...
    {
        MakeMainLidLabelStriped(width = width, length = length, lid_thickness = lid_thickness, label = label,
